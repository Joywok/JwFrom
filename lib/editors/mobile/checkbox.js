...
import {Checkbox, TextareaItem} from 'jw-components-mobile';
  ...
  _init_layout(){
	...
	render(){
		...
		if(schema["remote"] && (!schema['options'] || schema['options'].length==0)){
			...
			target = <div className="checkbox-list" {...schema['attr']}>
				{
					_.map(schema.options,function(item,index){
						...
						if(item.hasInput){
							let inputClassName = "checkbox-textarea " + item.className
							return <div className="checkbox-list-i-1">
								<CheckboxItem key={item.value} disabled={item["disabled"] || false} checked={is_has} onChange={(e)=>self.onChange(e,item,schema)}> <span>{item.label}</span></CheckboxItem>
