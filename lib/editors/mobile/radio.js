	...
	render(){
		...
		for(var i in schema.options){
			if(schema.options[i].hasInput){
				let inputClassName = "radio-input " + schema.options[i].className
				if(schema.options[i].inputDisabled === false){
					input = <InputItem className={inputClassName}  onChange={this.inputChange.bind(this)} value={schema.options[i].inputValue}/>
				}else{
					input = <InputItem className={inputClassName} disabled onChange={this.inputChange.bind(this)} value={schema.options[i].inputValue}/>
				}
