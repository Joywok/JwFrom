...
class Radios extends Component {
  ...
  _init_layout(){
	...
	render(){
		let schema = this.props.schema;
		let self = this;
		let target = <div className="radio-list">
			{
				_.map(schema.options,function(item){
					return <RadioItem key={item.value} className="radio-list-i" checked={schema.defaultValue == item.value?true:false} onChange={()=>self.onChange(item.value,schema)}>{item.label}</RadioItem>
				})
			}
		</div>;
