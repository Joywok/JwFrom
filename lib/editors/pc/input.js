...
class InputCustom extends Component {
	...
	render(){
		...
		if(this.props.type && this.props.type == 'Textarea'){
			...
			target = <TextArea {...data} onChange={(e)=>this.onChange(e,schema)} value={schema['defaultValue']}/>;
		}else{
			target = <Input {...data} onChange={(e)=>this.onChange(e,schema)} value={schema['defaultValue']}/>;
		}
